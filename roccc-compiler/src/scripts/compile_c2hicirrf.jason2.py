...
load $1.suif ;
require global_transforms array_transforms bit_vector_dataflow_analysis;
require gcc_preprocessing_transforms loop_transforms utility_transforms ;
dismantle_call_expressions ;
PreprocessPass ;
require preprocessing_transforms ;
# SwitchCaseToIfStmtsPass ;
HW_SW_BoundaryMarkPass ;
Process_LUT_DecsPass ;
RemoveLoopLabelLocStmtsPass ;
FlattenScopeStatementsPass ;
FlattenStatementListsPass ;
ControlFlowSolvePass ;
DataFlowSolvePass ;
UD_DU_ChainBuilderPass ;
ConstantPropagationAndFoldingPass ;
StripAnnotesPass ;
### INSERT FULLY_UNROLL_STATEMENT ;
### INSERT GENERATE_TILE_STATEMENT ;
### INSERT PARTIALLY_UNROLL_STATEMENT  ;
### INSERT GENERATE_SYSTOLIC_ARRAY_UNROLL_STATEMENT ;
FlattenStatementListsPass ;
IfConversionPass ;
save $1.suif ; """ , 
"""\
require basicnodes suifnodes cfenodes transforms loop_transforms control_flow_analysis ;
load $1.suif ;
require bit_vector_dataflow_analysis global_transforms utility_transforms array_transforms optimizer_output;
ControlFlowSolvePass ;
DataFlowSolvePass ;
UD_DU_ChainBuilderPass ;
ConstantPropagationAndFoldingPass;
...
ConstantPropagationAndFoldingPass ;
DivByConstEliminationPass ;
MultiplyByConstEliminationPass ;
ConstantPropagationAndFoldingPass ;
UD_DU_ChainBuilderPass ;
CopyPropagationPass ;
...
### INSERT GENERATE_SYSTOLIC_ARRAY_INTERCHANGE_STATEMENT ;
FusePass;
InvariantCodeMotionPass ;
UnreachableCodeEliminationPass ;
FlattenStatementListsPass ;
ControlFlowSolvePass ;
DataFlowSolvePass;
UD_DU_ChainBuilderPass ;
ScalarIODetectionPass ;
StripAnnotesPass ;
save $1.suif ; """,
"""\
require basicnodes suifnodes cfenodes transforms control_flow_analysis utility_transforms;
load $1.suif
require bit_vector_dataflow_analysis global_transforms array_transforms optimizer_output fp_accumulate_transform ;
RawEliminationPass ;
ScalarReplacementPass ;
name_all_symbols ;
FlattenStatementListsPass ;
MEM_DP_BoundaryMarkPass ;
require data_dependence_analysis ;
LoopInfoPass ;
PreprocessingPass ;
### INSERT GENERATE_SYSTOLIC_ARRAY_FEEDBACK_ELIMINATE_STATEMENT ;
name_all_symbols ;
LoopInfoPass ;
PreprocessingPass ;
FlattenStatementListsPass ;
ReductionParallelizationPass ;
FlattenStatementListsPass ;
ControlFlowSolvePass ;      
DataFlowSolvePass ;
UD_DU_ChainBuilderPass ;   
...
DataFlowSolvePass ;
UD_DU_ChainBuilderPass ;
SolveFeedbackVariablesPass ;
SolveFeedbackVariables2Pass ;
AccumulatePass ;
BufferTypeDetectionPass ;
DatapathGenerationPass ;
HostCodeGenerationPass ;
StripAnnotesPass;
save $1.suif ;
""" ]
    ...
    for the_split_input_line in split_passes:
      ...
      #################
        ...
        if len( PASS_MAP[CONST_STR_GEN_SYS_ARRAY_STMT] ) > 0 : 
          new_passes.append(the_line + NEWLINE)
          for p in PASS_MAP[CONST_STR_GEN_SYS_ARRAY_STMT]:
            temp_str = 'FeedbackEliminationPass 1 ;' + NEWLINE
            new_passes.append(temp_str)
...
if __name__ == "__main__":
  ...
  if retval != 0 :
    error_print(ERROR_GCC2SUIF, 'GCC2SUIF COMPILATION ERROR(retval=%d)'%retval)
  ...
  for pass_list in CONST_NEW_SUIFDRIVER_PASSES_LIST:
    ...
    if retval != 0 :
      error_print(ERROR_SUIFDRIVER_EXECUTE, 'SUIFDRIVER EXECUTION ERROR (retval=%d)'%retval)
