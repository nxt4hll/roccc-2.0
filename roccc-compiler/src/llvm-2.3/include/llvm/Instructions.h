...
public:
  ...
  // Additional Instruction For ROCCC
  /*  class CopyInstruction : public UnaryInstruction
  {
  private:
  public:
    CopyInstruction(const Type* Ty, Value* V, Instruction* insertBefore = 0) ;
    //    ~CopyInstruction() ;
    
    static inline bool classof (const CopyInstruction*) { return true ;}
    static inline bool classof(const Instruction* i) 
    { return i->getOpcode() == 45 ; } // 45 is UserDefined1

    CopyInstruction* clone() const ; // Don't actually call...it is broken.
